 ...
 *
 */
int* dynamicArray(int n, int qr, int qc, int** q, int* result_count) {
    int la=0,i,j,x,y,**arr,indx,k=0;
    ...
    *result_count=0;
    ...
    for(i=0;i<qr;i++)
    {
        if(q[i][0]==1)
        {
            ...
            c[indx]++;
            if(c[indx]==1)
            *(arr+indx)=malloc(c[indx]*sizeof(int));
            else {
            *(arr+indx)=realloc(*(arr+indx),c[indx]*sizeof(int));
            }
            arr[indx][c[indx]-1]=y;
        }
        else 
